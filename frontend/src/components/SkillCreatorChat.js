...
import { skillCreatorApi, buildSkillMarkdown } from "../api/skillCreator";
...
export default function SkillCreatorChat({ userId, scope, onClose, onPublished, embedded = false, editSkillName }) {
    ...
    const panel = (_jsxs("div", { className: `bg-white/95 backdrop-blur-xl rounded-2.5xl shadow-panel w-full flex flex-col border border-ink-200/60 ${embedded ? "h-[70vh]" : "max-w-4xl h-[90vh]"}`, children: [_jsxs("div", { className: "px-6 py-4 border-b border-ink-200/60 flex items-center justify-between shrink-0", children: [_jsxs("div", { children: [_jsx("h2", { className: "font-semibold text-ink-900", children: isEditMode ? `编辑 Skill：${editSkillName}` : `对话创建${scopeLabel}` }), _jsx("p", { className: "text-xs text-ink-400 mt-0.5", children: isEditMode
                                    ...
                                    openSession(true);
                                                ...
                                                : "bg-ink-100/80 text-ink-800 border border-ink-200/60"}`, children: m.role === "user" ? m.content : _jsx(ChatMarkdown, { content: m.content }) }) }, i))), sending && _jsx("p", { className: "text-xs text-gray-400", children: "\u52A9\u624B\u601D\u8003\u4E2D\u2026" }), _jsx("div", { ref: bottomRef })] }), error && (_jsx("p", { className: "mx-4 mb-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2", children: error })), uploads.length > 0 && (_jsx("div", { className: "px-4 pb-2 flex flex-wrap gap-1.5", children: uploads.map((u) => (_jsx("span", { className: "inline-flex items-center text-xs px-2 py-1 rounded-lg bg-ink-50 text-ink-600 border border-ink-200/60", title: `${u.skill_dir}/${u.relative_path}`, children: u.filename }, `${u.skill_dir}/${u.relative_path}`))) })), _jsxs("div", { className: "px-4 py-3 border-t flex gap-2 items-end shrink-0", children: [_jsx("input", { ref: fileInputRef, type: "file", className: "hidden", onChange: (e) => void handleUpload(e.target.files?.[0]) }), _jsx("button", { type: "button", title: "\u4E0A\u4F20\u9644\u4EF6\u5230 Skill \u76EE\u5F55", disabled: loading || !sessionId || uploading || sending, onClick: () => fileInputRef.current?.click(), className: "shrink-0 self-end text-sm px-2.5 py-1.5 rounded-lg border border-ink-200 text-ink-600 hover:bg-ink-50 disabled:opacity-40 disabled:cursor-not-allowed", children: uploading ? "…" : "附件" }), _jsx("textarea", { ref: textareaRef, rows: 1, value: input, onChange: (e) => setInput(e.target.value), onKeyDown: handleKeyDown, placeholder: "\u63CF\u8FF0\u4F60\u60F3\u521B\u5EFA\u7684 Skill\u2026", disabled: loading || !sessionId, className: "flex-1 resize-none bg-transparent border border-ink-200/80 rounded-xl px-3 py-2 text-sm text-ink-900 placeholder:text-ink-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-300 disabled:opacity-60 leading-relaxed" }), sending ? (_jsx("button", { type: "button", onClick: handleInterrupt, className: "ui-btn-danger shrink-0 self-end", children: "\u4E2D\u65AD" })) : (_jsx("button", { type: "button", onClick: handleSend, disabled: loading || !sessionId || !input.trim(), className: "ui-btn-primary shrink-0 self-end", children: "\u53D1\u9001" }))] })] }), _jsxs("div", { className: "w-80 flex flex-col shrink-0 bg-gray-50", children: [_jsxs("div", { className: "px-4 py-3 border-b", children: [_jsx("h3", { className: "text-sm font-medium text-gray-700", children: "\u8349\u7A3F\u9884\u89C8" }), _jsx("p", { className: "text-xs text-gray-500 mt-0.5", children: "\u7EE7\u7EED\u5BF9\u8BDD\u5B8C\u5584\uFF0C\u5B9A\u7A3F\u540E\u4FDD\u5B58" })] }), _jsx("div", { className: "flex-1 overflow-y-auto px-4 py-3 text-xs", children: !draft ? (_jsx("p", { className: "text-gray-400 text-center py-8", children: "\u5BF9\u8BDD\u751F\u6210 Skill \u540E\u663E\u793A\u5728\u6B64" })) : (_jsx("div", { className: "bg-white border rounded-lg p-3 max-h-full overflow-y-auto", children: _jsx(ChatMarkdown, { content: buildSkillMarkdown(draft) }) })) }), _jsx("div", { className: "px-4 py-3 border-t", children: _jsx("button", { onClick: handlePublish, disabled: !draft || publishing, className: "w-full ui-btn-primary", children: publishing ? "保存中…" : "保存 Skill" }) })] })] })] }));
    if (embedded)
        return panel;
