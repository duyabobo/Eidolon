...
export function preprocessConnectionLines(content: string, graphNodes: WikiGraphNode[]): string {
  return content
    .split("\n")
    .map((line) => {
      ...
      if (/<\/?[a-z][\w:-]*\b/i.test(trimmed)) return line;

      const parsed = parseConnectionLine(trimmed);
      if (!parsed.description) return line;
