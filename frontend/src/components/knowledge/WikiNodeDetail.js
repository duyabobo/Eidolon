...
function collectConnectionInputs(node) {
    const inputs = [...(node.connections ?? [])];
    const linksSection = node.body_sections["链接"] ??
...
export default function WikiNodeDetail({ node, loading, error, graphNodes, graphEdges = [], onNavigateNode, onClose, }) {
    ...
    const bodySections = node
        ? Object.entries(node.body_sections).filter(([key, value]) => {
            if (!value?.trim())
                return false;
            if (key.toLowerCase() === "connections" || key === "链接")
                return false;
