...
function resolveNodeId(conn, labelHint, byId, byTitle) {
...
function pushLink(results, seen, nodeId, label, description, byId, currentNodeId) {
...
export function resolveWikiConnections(connections, graphNodes, graphEdges = [], currentNodeId) {
    ...
    if (currentNodeId) {
        for (const edge of graphEdges) {
            ...
            pushLink(results, seen, edge.target_id, target?.title ?? edge.target_id, edge.description, byId, currentNodeId);
        }
    }
    return results;
}
export function connectionsToMarkdown(links) {
    if (!links.length)
        return "";
    return links
        .map((link) => {
