...
export function isCiteRefHref(href?: string): boolean {
  return Boolean(href?.startsWith(CITE_REF_PREFIX));
}

export function citeRefNumberFromHref(href: string): number {
  return Number(href.slice(CITE_REF_PREFIX.length));
}

/** 解析「参考来源」列表，并将正文中的 [1] 转为可点击上标链接 */
export function parseChatCitations(content: string): {
  markdown: string;
  ...
  const refs = new Map<number, CitationRef>();
