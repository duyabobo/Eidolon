...
export default function SkillsPanel({ userId, onSkillsChanged }) {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [msg && (_jsx("p", { className: `text-sm px-3 py-2 rounded-lg ${msg.type === "ok" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`, children: msg.text })), _jsxs("div", { className: "space-y-2", children: [skills.length === 0 && (_jsx("p", { className: "text-sm text-ink-400 text-center py-8 border border-dashed border-ink-200 rounded-xl", children: "\u6682\u65E0 Skill" })), skills.map((s) => {
