...
from services.llm_config_store import activate_profile, get_effective_config
...
async def list_llm_profiles() -> LlmProfileListResponse:
    items, active_id = await mongo_client.list_llm_profiles()
...
async def create_llm_profile(body: LlmProfileCreate) -> LlmProfile:
    profile = await mongo_client.create_llm_profile(body)
    items, active_id = await mongo_client.list_llm_profiles()
    if not active_id:
...
async def update_llm_profile(profile_id: str, body: LlmProfileUpdate) -> LlmProfile:
    updated = await mongo_client.update_llm_profile(profile_id, body)
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="LLM 配置不存在")
    _, active_id = await mongo_client.list_llm_profiles()
...
async def delete_llm_profile(profile_id: str) -> None:
    items, active_id = await mongo_client.list_llm_profiles()
    if len(items) <= 1:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="至少保留一个 LLM 配置")
    if not await mongo_client.delete_llm_profile(profile_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="LLM 配置不存在")
    if active_id == profile_id:
