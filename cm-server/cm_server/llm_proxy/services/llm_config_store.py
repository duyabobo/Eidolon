...
from cm_server.llm_proxy.config import settings
from cm_server.llm_proxy.models.config import LlmConfig, LlmProfile
...
async def load_from_db() -> None:
    """启动时调用，从本地 SQLite 加载当前选中的 LLM 配置到内存"""
    global _current
    await llm_profile_store.migrate_legacy_llm_config()
    active = await llm_profile_store.get_active_llm_profile()
    if active:
...
async def activate_profile(profile_id: str) -> LlmConfig:
    ...
    await llm_profile_store.set_active_llm_profile(profile_id)
