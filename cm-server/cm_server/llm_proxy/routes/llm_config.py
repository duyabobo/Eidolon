...
async def list_llm_profiles() -> LlmProfileListResponse:
    items, active_id, intent_id = await llm_profile_store.list_llm_profiles()
    return LlmProfileListResponse(items=items, active_id=active_id, intent_id=intent_id)
...
async def create_llm_profile(body: LlmProfileCreate) -> LlmProfile:
    ...
    items, active_id, _intent_id = await llm_profile_store.list_llm_profiles()
    if not active_id:
...
async def update_llm_profile(profile_id: str, body: LlmProfileUpdate) -> LlmProfile:
    ...
    _, active_id, _intent_id = await llm_profile_store.list_llm_profiles()
...
async def delete_llm_profile(profile_id: str) -> None:
    ...
    if not await llm_profile_store.delete_llm_profile(profile_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="LLM 配置不存在")
    if intent_id == profile_id:
        await llm_profile_store.set_intent_llm_profile(None)
    if active_id == profile_id:
...
@router.put("/llm/intent", response_model=LlmProfileListResponse)
async def assign_intent_llm_profile(body: LlmIntentAssign) -> LlmProfileListResponse:
    try:
        await llm_profile_store.set_intent_llm_profile(body.profile_id)
