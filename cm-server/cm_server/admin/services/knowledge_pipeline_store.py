"""知识库流水线配置（mineru），存 app_config.knowledge_pipeline。"""
...
def _row_to_config(raw: str | None) -> KnowledgePipelineConfig:
    ...
    return KnowledgePipelineConfig(
        ...
        mineru3_api_key=str(data.get("mineru3_api_key") or ""),
...
async def save_pipeline_config(cfg: KnowledgePipelineConfig) -> KnowledgePipelineConfig:
    ...
    payload = KnowledgePipelineConfig(
        ...
        mineru3_api_key=(cfg.mineru3_api_key or "").strip(),
    ...
    logger.info("知识库流水线配置已保存 mineru=%s", payload.mineru3_api_base)
    return payload
