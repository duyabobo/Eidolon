...
async def api_list_bases(
    ...
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.list_bases(page, page_size)
...
async def api_create_base(body: KnowledgeBaseCreate) -> KnowledgeBase:
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.create_base(body)
...
async def api_get_base(kb_id: str) -> KnowledgeBase:
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.get_base(kb_id)
...
async def api_update_base(kb_id: str, body: KnowledgeBaseUpdate) -> KnowledgeBase:
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.update_base(kb_id, body)
...
async def api_delete_base(kb_id: str) -> None:
    if await knowledge_config_store.is_remote_mode():
        await knowledge_client.delete_base(kb_id)
        return
...
async def api_list_documents(
    ...
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.list_documents(kb_id, page, page_size)
...
async def api_upload_document(kb_id: str, file: UploadFile = File(...)) -> KnowledgeDocument:
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.upload_document(kb_id, file)
...
async def api_get_document(kb_id: str, doc_id: str) -> KnowledgeDocument:
    if await knowledge_config_store.is_remote_mode():
        return await knowledge_client.get_document(kb_id, doc_id)
...
async def api_delete_document(kb_id: str, doc_id: str) -> None:
    if await knowledge_config_store.is_remote_mode():
        await knowledge_client.delete_document(kb_id, doc_id)
        return
